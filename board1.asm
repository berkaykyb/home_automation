;===============================================================================
;   HOME AUTOMATION SYSTEM - AIR CONDITIONER CONTROL
;   Microcontroller: PIC16F877A
;   Crystal: 4 MHz
;===============================================================================
;
;   DESCRIPTION:
;       Temperature control system with keypad input, 7-segment display,
;       and UART communication for PC interface.
;
;   FEATURES:
;       - LM35 temperature sensor with ADC
;       - 4x4 matrix keypad for temperature input (10-50 C)
;       - 4-digit multiplexed 7-segment display
;       - 2-degree hysteresis temperature control
;       - UART communication (9600 baud)
;       - Heater and fan control outputs
;
;   PIN CONFIGURATION:
;       RA0: LM35 Temperature Sensor (Analog Input)
;       RA1-RA3: Keypad Columns 1-3
;       RB0: Keypad Column 4
;       RB4-RB7: Keypad Rows 1-4
;       RC0: Heater Output
;       RC1: Fan/Cooler Output
;       RC2: Tachometer Input
;       RC3: Input Mode LED (Blue)
;       RC4: Success LED (Green)
;       RC5: Error LED (Red)
;       RC6-RC7: UART TX/RX
;       RD0-RD7: 7-Segment Display Data
;       RE0-RE2: Digit Select (MUX)
;
;===============================================================================
...
DELAY_CNT2:         DS 1
DELAY_CNT3:         DS 1
HEATER_STATE:       DS 1    ; 0=idle/reached target, 1=heating towards target
...
MAIN:
    ...
    ; Default desired temperature 27.4 C
    MOVLW   27
    ...
    ; Default ambient temperature 27.4 C (until first ADC read)
    MOVLW   27
...
;==============================================================================
MAIN_LOOP:
    ; Check for UART commands FIRST (for faster response)
    ...
    ; Added UART checks between for responsive communication
    CALL    UPDATE_DISPLAY
    CALL    DELAY_5MS
    CALL    CHECK_UART_COMMAND
    CALL    UPDATE_DISPLAY
...
;==============================================================================
; INIT_PORTS - Configure all I/O pin directions and initial states
;==============================================================================
INIT_PORTS:
...
;==============================================================================
; INIT_ADC - Configure ADC for LM35 temperature sensor reading
;==============================================================================
INIT_ADC:
    banksel ADCON0
...
;==============================================================================
; INIT_VARIABLES - Reset all system RAM variables to default values
;==============================================================================
INIT_VARIABLES:
    banksel INPUT_MODE
    CLRF    INPUT_MODE
...
;==============================================================================
; READ_TEMPERATURE - Read LM35 sensor via ADC and convert to degrees Celsius
;==============================================================================
READ_TEMPERATURE:
    banksel ADCON0
...
WAIT_ADC:
    ...
    ; Read 10-bit result (ADRESH:ADRESL)
    MOVF    ADRESH, W
    ...
    ; More accurate calculation: Temp = ADC * 500 / 1024
    ; Approximation: Temp = (ADC / 2) - (ADC / 32) = ADC * 0.46875
    ; Even simpler for PICSimLab: ADC / 2 gives close result
    ; but we need to subtract about 1 to correct the offset
    ...
    MOVWF   TEMP_REG
    BCF     STATUS, 0
    RRF     TEMP_REG, F         ; TEMP_REG = ADC_L / 2
    
    banksel ADC_RESULT_H
    BTFSC   ADC_RESULT_H, 0     ; If ADC_H bit 0 is set
    BSF     TEMP_REG, 7         ; Set bit 7 of result (+128)
    
    ; Correction: subtract 1 to fix offset (ADC/2 gives ~1 degree high)
    banksel TEMP_REG
    MOVF    TEMP_REG, W
    SUBLW   1                   ; Check if TEMP_REG > 1
    BTFSS   STATUS, 0           ; If TEMP_REG <= 1, skip
    DECF    TEMP_REG, F         ; Subtract 1 from result
    
    MOVF    TEMP_REG, W
    banksel AMBIENT_TEMP_INT
    MOVWF   AMBIENT_TEMP_INT
    
    ; Fractional part: Use lower 4 bits of ADC for 0.1 precision
    ; ADC lower 4 bits (0-15) -> map to 0-9
    ; Method: (ADC_L & 0x0F) * 10 / 16 ≈ (ADC_L & 0x0F) * 5 / 8
    ; Simpler: (ADC_L & 0x0F) / 2, then cap at 9
    banksel ADC_RESULT_L
    MOVF    ADC_RESULT_L, W
    ANDLW   0x0F                ; Lower 4 bits (0-15)
    banksel TEMP_REG
    MOVWF   TEMP_REG
    BCF     STATUS, 0
    RRF     TEMP_REG, F         ; Divide by 2 (0-7)
    ...
    ; Cap at 9
    MOVLW   10
    SUBWF   AMBIENT_TEMP_FRAC, W
    BTFSC   STATUS, 0           ; If FRAC >= 10
    GOTO    CAP_FRAC_9
    RETURN
...
;==============================================================================
; UPDATE_FAN_SPEED - Set fan speed value based on RC1 output state
;==============================================================================
UPDATE_FAN_SPEED:
    banksel PORTC
...
;==============================================================================
; CONTROL_TEMPERATURE - Heater/fan control with 2-degree hysteresis algorithm
;==============================================================================
CONTROL_TEMPERATURE:
    ;------------------------------------------------------------------
    ; STEP 1: Check if COOLING needed (Ambient > Desired + 0.2)
    ; Compare: Ambient_int vs Desired_int, then fractional with +2 offset
    ;------------------------------------------------------------------
    banksel DESIRED_TEMP_INT
    MOVF    DESIRED_TEMP_INT, W
    banksel AMBIENT_TEMP_INT
    SUBWF   AMBIENT_TEMP_INT, W     ; W = Ambient_int - Desired_int
    
    BTFSS   STATUS, 0               ; C=0? (Ambient_int < Desired_int)
    GOTO    CHECK_TARGET_REACHED    ; No cooling needed
    
    BTFSC   STATUS, 2               ; Z=1? (Ambient_int = Desired_int)
    GOTO    CHECK_FRAC_COOL         ; Check fractions
    
    ; Ambient_int > Desired_int -> COOL
    GOTO    CTRL_COOL

CHECK_FRAC_COOL:
    ; Ambient_int = Desired_int, check if Ambient_frac > Desired_frac + 2
    banksel DESIRED_TEMP_FRAC
    MOVF    DESIRED_TEMP_FRAC, W
    ADDLW   2                       ; W = Desired_frac + 2 (0.2 tolerance)
    banksel CALC_TEMP
    MOVWF   CALC_TEMP               ; CALC_TEMP = Desired_frac + 2
    
    ; Handle overflow: if CALC_TEMP >= 10, cooler definitely needed
    MOVLW   10
    SUBWF   CALC_TEMP, W
    BTFSC   STATUS, 0               ; C=1 if CALC_TEMP >= 10
    GOTO    CHECK_TARGET_REACHED    ; Frac overflow, compare normally
    
    ; Compare Ambient_frac with threshold
    banksel CALC_TEMP
    MOVF    CALC_TEMP, W
    banksel AMBIENT_TEMP_FRAC
    SUBWF   AMBIENT_TEMP_FRAC, W    ; W = Ambient_frac - (Desired_frac+2)
    
    BTFSS   STATUS, 0               ; C=0? (Ambient_frac < threshold)
    GOTO    CHECK_TARGET_REACHED    ; No cooling needed
    
    BTFSC   STATUS, 2               ; Z=1? (equal)
    GOTO    CHECK_TARGET_REACHED    ; At threshold, no cooling yet
    
    ; Ambient_frac > threshold -> COOL
    GOTO    CTRL_COOL

CHECK_TARGET_REACHED:
    ;------------------------------------------------------------------
    ; Check if Ambient > Desired + 0.1 (heater stop condition)
    ;------------------------------------------------------------------
    banksel DESIRED_TEMP_INT
    MOVF    DESIRED_TEMP_INT, W
    banksel AMBIENT_TEMP_INT
    ...
    BTFSS   STATUS, 0               ; C=0? (Ambient_int < Desired_int)
    GOTO    CHECK_HEATER_STATE      ; Yes, check heating logic
    
    BTFSC   STATUS, 2               ; Z=1? (Ambient_int = Desired_int)
    GOTO    CHECK_FRAC_TARGET       ; Check fractional for +0.1
    
    ; Ambient_int > Desired_int, target definitely reached
    banksel HEATER_STATE
    BCF     HEATER_STATE, 0
    GOTO    CTRL_OFF

CHECK_FRAC_TARGET:
    ; Check if Ambient_frac > Desired_frac + 1 (0.1 tolerance)
    banksel DESIRED_TEMP_FRAC
    MOVF    DESIRED_TEMP_FRAC, W
    ADDLW   1                       ; W = Desired_frac + 1
    banksel CALC_TEMP
    MOVWF   CALC_TEMP
    ...
    SUBWF   CALC_TEMP, W
    BTFSC   STATUS, 0               ; C=1 if CALC_TEMP >= 10
    GOTO    TARGET_STOP             ; Overflow means target reached
    
    ; Compare
    banksel CALC_TEMP
    MOVF    CALC_TEMP, W
    banksel AMBIENT_TEMP_FRAC
    SUBWF   AMBIENT_TEMP_FRAC, W    ; W = Ambient_frac - (Desired_frac+1)
    
    BTFSS   STATUS, 0               ; C=0? (Ambient_frac < threshold)
    GOTO    CHECK_HEATER_STATE      ; Not yet at target+0.1, continue heating
    
    ; Ambient_frac >= threshold, target reached
TARGET_STOP:
    banksel HEATER_STATE
    BCF     HEATER_STATE, 0
    GOTO    CTRL_OFF
...
CHECK_HEATER_STATE:
    ;------------------------------------------------------------------
    ; STEP 2: Ambient < Desired, check heating logic based on state
    ;------------------------------------------------------------------
    banksel HEATER_STATE
    BTFSC   HEATER_STATE, 0         ; HEATER_STATE = 1?
    GOTO    HEATING_MODE            ; Yes, continue heating

IDLE_MODE:
    ;------------------------------------------------------------------
    ; IDLE: Heater only turns ON when Ambient <= Desired-2
    ;------------------------------------------------------------------
    banksel DESIRED_TEMP_INT
    MOVF    DESIRED_TEMP_INT, W
    banksel CALC_TEMP
    ...
    MOVLW   2
    SUBWF   CALC_TEMP, F            ; CALC_TEMP = DESIRED - 2
    
    banksel CALC_TEMP
    MOVF    CALC_TEMP, W
    banksel AMBIENT_TEMP_INT
    SUBWF   AMBIENT_TEMP_INT, W     ; W = Ambient - Threshold
    
    BTFSS   STATUS, 0               ; C=0? (Ambient < Threshold)
    GOTO    START_HEATING           ; Need to start heating
    
    BTFSC   STATUS, 2               ; Z=1? (Ambient = Threshold)
    GOTO    CHECK_FRAC_START        ; Check fractions
    
    ; Ambient > Threshold, stay idle
    GOTO    CTRL_OFF
...
CHECK_FRAC_START:
    ...
    BTFSS   STATUS, 0               ; Ambient_frac < Desired_frac?
    GOTO    START_HEATING
...
START_HEATING:
    ; Set HEATER_STATE = 1 and turn heater ON
    banksel HEATER_STATE
    BSF     HEATER_STATE, 0
    GOTO    CTRL_HEAT
...
HEATING_MODE:
    ;------------------------------------------------------------------
    ; HEATING MODE: Stay ON until Ambient >= Desired
    ; (Already checked above that Ambient < Desired, so keep heating)
    ;------------------------------------------------------------------
    GOTO    CTRL_HEAT
...
CTRL_COOL:
    ; Also reset HEATER_STATE when cooling
    banksel HEATER_STATE
    BCF     HEATER_STATE, 0
    banksel PORTC
...
;==============================================================================
; PROCESS_KEYPAD - Scan and process keypad input for temperature entry
;==============================================================================
PROCESS_KEYPAD:
    CALL    SCAN_KEYPAD
...
;==============================================================================
; ENTER_INPUT_MODE - Prepare system for keypad temperature entry
;==============================================================================
ENTER_INPUT_MODE:
    banksel INPUT_MODE
    BSF     INPUT_MODE, 0
...
;==============================================================================
; SCAN_KEYPAD - Scan 4x4 matrix keypad and detect pressed key
;==============================================================================
SCAN_KEYPAD:
    banksel KEY_VALUE
    MOVLW   KEY_NONE
...
;==============================================================================
; UPDATE_DISPLAY - 4-digit multiplexed 7-segment display driver
;==============================================================================
UPDATE_DISPLAY:
...
;==============================================================================
; UART_SEND_BYTE - Send one byte through UART transmitter
;==============================================================================
UART_SEND_BYTE:
    banksel TXREG
...
;==============================================================================
; CHECK_UART_COMMAND - Process incoming UART commands from PC
;==============================================================================
CHECK_UART_COMMAND:
    ; Check for UART errors first (like board2.asm)
    banksel RCSTA
    BTFSC   RCSTA, 1            ; OERR - Overrun Error?
    GOTO    FIX_UART_OERR
    BTFSC   RCSTA, 2            ; FERR - Framing Error?
    GOTO    DISCARD_UART_FERR
...
;------------------------------------------------------------------------------
; UART Error Recovery Routines
;------------------------------------------------------------------------------
FIX_UART_OERR:
    ; Clear overrun error by cycling CREN
    banksel RCSTA
    BCF     RCSTA, 4            ; CREN = 0
    BSF     RCSTA, 4            ; CREN = 1
    RETURN
...
DISCARD_UART_FERR:
    ; Discard byte with framing error by reading RCREG
    banksel RCREG
    MOVF    RCREG, W            ; Read and discard
    RETURN
